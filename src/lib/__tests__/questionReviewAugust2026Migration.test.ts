...
describe("August 2026 question review migration", () => {
  it("locks the approved question updates and guarded fallbacks", async () => {
    ...
    const updates = [
      {
        ...
        sport: "cbb",
        priorText: "Which team beat UCLA in the 1974 national semifinals before winning the title?",
        finalText:
      ...
      },
      {
        ...
        sport: "cfb",
        priorText: "Which matchup was the first college football game broadcast on radio?",
        finalText:
      ...
      },
      {
        ...
        sport: "cbb",
        priorText: "Which school won the 1997 national title behind freshman Mike Bibby?",
        finalText:
      ...
      },
      {
        ...
        sport: "cfb",
        priorText: "Which program owns the major-college record 47-game winning streak?",
        finalText:
      ...
      },
      {
        ...
        sport: "cbb",
        priorText: "Who is the all-time leading scorer in Division I men's basketball?",
        finalText:
    ...
    for (const update of updates) {
      expect(migration).toContain(update.id);
      expect(migration).toContain(update.finalText);
      expect(migration).toMatch(
        new RegExp(
          `s\\.slug = '${update.sport}'[\\s\\S]*q\\.id = '${update.id}'::uuid[\\s\\S]*q\\.question_text = '${update.priorText.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&")}'`,
        ),
      );
    }

    expect(migration).toContain("difficulty = 'medium'");
    expect(migration.match(/difficulty = 'medium'/g)).toHaveLength(2);
    expect(migration).toContain("source_notes =");
    expect(migration).toContain("reviewed_at = timezone('utc', now())");
    expect(migration).toContain("updated_at = timezone('utc', now())");
    expect(migration.match(/get diagnostics updated_count = row_count;/g)).toHaveLength(8);
    expect(migration.match(/if updated_count <> 1 then/g)).toHaveLength(8);
    expect(migration.match(/raise exception/g)).toHaveLength(8);
